                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($metrics as $metric)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">
                                                    {{ $metric->performed_at ? $metric->performed_at->format('M d, Y') : 'N/A' }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('metrics.show', $metric) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                                <a href="{{ route('metrics.edit', $metric) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                                <form action="{{ route('metrics.destroy', $metric) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this metric?')">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
