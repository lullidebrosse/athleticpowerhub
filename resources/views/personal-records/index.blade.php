                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($prs as $pr)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">
                                                    {{ $pr->achieved_at->format('M d, Y') }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('personal-records.show', $pr) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                                <form action="{{ route('personal-records.destroy', $pr) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this personal record?')">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
